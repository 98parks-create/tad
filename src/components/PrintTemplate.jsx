...
const PrintTemplate = forwardRef(({ customerInfo, items, subTotal, discount, discountReason, vat, grandTotal, providerInfo, remarks, includeVat = true, attachedImages = [] }, ref) => {
  ...
  // 1. 공급가액(total)이 큰 순서대로 정렬 추가
  const sortedItems = [...items].sort((a, b) => (Number(b.total) || 0) - (Number(a.total) || 0));
          ...
          <table style={{ width: '380px', borderCollapse: 'collapse', fontSize: '11.5pt', border: '2px solid #000' }}>
            <tbody>
              ...
              {(discount || 0) > 0 && (
                <>
                  <tr>
                    <td style={{ border: '1px solid #000', borderBottom: discountReason ? 'none' : '1px dotted #666', padding: '8px', backgroundColor: '#fef3c7', fontWeight: 'bold' }}>할인액</td>
                    <td style={{ border: '1px solid #000', borderBottom: discountReason ? 'none' : '1px dotted #666', padding: '8px', textAlign: 'right', color: '#b45309', fontWeight: 'bold', verticalAlign: 'middle' }}>- {Number(discount || 0).toLocaleString()} 원</td>
                  </tr>
                  {discountReason && (
                    <tr>
                      <td colSpan="2" style={{ border: '1px solid #000', borderTop: 'none', borderBottom: '1px dotted #666', padding: '0 8px 8px 8px', backgroundColor: '#fef3c7', textAlign: 'right', fontSize: '0.9em', color: '#b45309' }}>
                        {discountReason.includes('(') ? discountReason : `(${discountReason})`}
                      </td>
                    </tr>
                  )}
                </>
